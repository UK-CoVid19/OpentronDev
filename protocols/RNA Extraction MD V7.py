...
reagents['isopropanol_400'] = {'well': 'A12', 
                               'transfer_volume': 400, 
                               'mix_volume': 190, 
                               'mix_repetitions': MIX_REPETITIONS,
                               'new_tip': NEW_TIP_MODE}
...
def resuspend(well_to_mix):
    ...
    for position in np.arange(1,0.2, -0.2):
        print(position)
...
def transfer_and_mix(reagent, samples):
    ...
    for s in samples:
        ...
        m300.transfer(reagent['transfer_volume'], reagent['setup'].bottom(0.2), s.top(-10), new_tip=reagent['new_tip'], air_gap=10)
...
def blow_air(mins, samples):
    ...
    #UPDATE this for multiple rows - must take less time in between each sample than 14s.
    repetitions = int((mins*60)/(14*number_of_sample_columns))
...
for well in samples:
    ...
    m300.transfer(reagents['isopropanol_400']['transfer_volume'], reagents['isopropanol_400']['setup'], well.top(-10), new_tip='never', air_gap = 10)
